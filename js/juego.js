...
/*PAMPA >>> agrego esta funcion para utilizarla luego
dentro de la funcion chequearSiGano() y que lo que hace
es tranformar en una cadena de caracteres el array de arrays
o matriz que le paso como parametro */

var matrizACadena = function(matriz){
	var cadenita = "";
	for(var i = 0; i < matriz.length; i++){
		for(var j = 0; j < matriz[i].length; j++){
			cadenita = cadenita + matriz[i][j];
		}
	}
	return cadenita;
}
...
function chequearSiGano() {
	var ordenGanador = "123456789";
	return matrizACadena(grilla) == ordenGanador;
}
...
function mostrarMovimientoAlMomento() {
	var b = movimientos.length-movSistema;
	document.getElementById("header").innerHTML = "<div class='jugando'>Movimientos:"+ b +"</div>";
}
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
    //COMPLETAR  !!! listo la completé! Pampa.-
	var resguardoPos1 = grilla[filaPos1][columnaPos1];
	grilla[filaPos1][columnaPos1] = grilla[filaPos2][columnaPos2];
	grilla[filaPos2][columnaPos2] = resguardoPos1;
